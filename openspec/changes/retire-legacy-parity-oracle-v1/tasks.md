...
- [ ] 2.3 Leave `tests/test_architecture.py` unchanged — its production-import guard against `legacy_source`/BBB is a permanent architecture check, not parity tooling.
...
- [ ] 5.1 Run the full native test suite and confirm it passes with `legacy_source/`, `parity/`, and the removed tests gone.
- [ ] 5.2 Run `openspec validate --strict` for the archived spec deltas.
