# open-trade-projection-v1 Specification

## Purpose

Define the stateless `POST /v1/strategy-evaluations/open-trade` contract: the immutable executed-trade receipt and its pre-MDS validation, delegation through a strategy-family open-trade adapter, plan-basis start-after-entry management, the confirmed-open caller precondition, and the post-target-bar desired protection and strategic close signal result.
