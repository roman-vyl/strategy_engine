...
Define the derived `atr_distance` indicator: its dependency on an earlier ATR feature, its multiplication calculation with strict positive-finite multiplier validation, its reuse of the ATR read/warmup/validity instead of duplicating work, and its API compatibility with the existing range contract.
