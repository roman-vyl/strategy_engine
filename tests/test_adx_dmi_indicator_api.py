    ...
    def load_range(self, market, time_range):
        ...
        bars = tuple(
            MarketBar(
                ...
                Decimal(str(101 + index + index % 3)),
                Decimal(str(99 + index - index % 2)),
                Decimal(str(100 + index + ((index % 5) - 2) * 0.2)),
                Decimal("10"),
            )
    ...
    return (
        ApplicationServices(
            ...
            evaluate_indicator_range=EvaluateIndicatorRange(
                indicator_registry, market_data, validate_plan
            ),
...
def test_adx_dmi_api_returns_three_decimal_text_series_with_one_market_read() -> None:
    ...
    with TestClient(create_app(services=app_services)) as client:
        ...
        assert body["series"]["adx_base_3"][:4] == [None] * 4
        assert isinstance(body["series"]["adx_base_3"][4], str)
        assert market_data.calls == 1


def test_invalid_adx_is_rejected_before_market_read() -> None:
    app_services, market_data = services()
    payload = {
        "market": {
            "ticker": "BTCUSDT.P",
