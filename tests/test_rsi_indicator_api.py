...
def test_indicator_range_api_returns_decimal_text_rsi() -> None:
    ...
    with TestClient(create_app(services=app_services)) as client:
        ...
        body = response.json()
        assert body["series"]["rsi_base_3"][:3] == [None, None, None]
        assert body["series"]["rsi_base_3"][3] == "100"
        assert body["validity"]["rsi_base_3"]["warmup_bars"] == 3
        assert market_data.calls == 1


def test_bad_rsi_is_rejected_before_market_read() -> None:
    app_services, market_data = services()
    payload = {
        "market": {
            "ticker": "BTCUSDT.P",
...
def test_mixed_ema_atr_rsi_plan_uses_one_market_read() -> None:
    app_services, market_data = services()
    payload = {
        "market": {
            "ticker": "BTCUSDT.P",
    ...
    with TestClient(create_app(services=app_services)) as client:
        response = client.post("/v1/indicator-evaluations/range", json=payload)
        assert response.status_code == 200
        assert set(response.json()["series"]) == {
            "ema_base_3",
            "atr_base_3",
            "rsi_base_3",
        }
